            ...
            <div className='flex flex-col items-center justify-center min-h-screen bg-zinc-950'>
                <form onSubmit={Submit} className='p-8 border border-zinc-800 bg-zinc-900 rounded-lg shadow-xl w-80'>
                   ...
                   />
                    ...
                    className='w-full p-2 mb-4 bg-zinc-800 border border-rose-950 text-white rounded focus:outline-none focus:border-zinc-500'
                    onChange={(e)=> setFormData({...formData, password: e.target.value})}
                    />

                    <button type='submit' className='w-full py-2 bg-rose-950 text-zinc-100 font-bold rounded hover:bg-zinc-300 transition-colors'>SUBMIT</button>

                    <p className="mt-6 text-zinc-500 text-xs text-center font-mono">
  NEW USER? <Link to="/signup" className="text-zinc-100 underline hover:text-white transition-colors">REGISTER HERE</Link>
</p>
